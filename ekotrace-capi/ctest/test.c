...
bool test_backend_piping(void) {
    ...
    ekotrace_error result = ekotrace_initialize(destination, DEFAULT_TRACER_SIZE, DEFAULT_TRACER_ID, &t);
...
bool test_event_recording(void) {
    ...
    uint8_t * destination = (uint8_t*)malloc(DEFAULT_TRACER_SIZE);
    ekotrace * t;
    ekotrace_error result = EKOTRACE_INITIALIZE(
            destination,
            DEFAULT_TRACER_SIZE,
            DEFAULT_TRACER_ID,
            &t,
...
bool test_merge(void) {
    ...
    ekotrace * ekotrace_a;
    ekotrace_error result = ekotrace_initialize(destination_a, DEFAULT_TRACER_SIZE, DEFAULT_TRACER_ID, &ekotrace_a);
    if (result != EKOTRACE_ERROR_OK) {
        passed = false;
    }
